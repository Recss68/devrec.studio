						...
						<div class="rating-stars" aria-label="{avgRating.toFixed(1)} / 5">
							{#each starList(avgRating) as filled}
								<svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true">
									...
									{:else}
										<polygon fill="none" stroke="currentColor" stroke-width="1.5" points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26"/>
									{/if}
								</svg>
							{/each}
						</div>
								...
								{#each starList(review.rating) as filled}
									<svg width="13" height="13" viewBox="0 0 24 24" aria-hidden="true">
										{#if filled}
											<polygon fill="currentColor" points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26"/>
										{:else}
											<polygon fill="none" stroke="currentColor" stroke-width="2" points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26"/>
										{/if}
									</svg>
								{/each}
							</div>
...
<style>
	...
	.reviews-google-link:hover { color: var(--c-fg); }
	...
	.review-card:hover { border-color: var(--c-fg); }
